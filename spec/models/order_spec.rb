...
RSpec.describe Order, type: :model do
  describe '購入情報の保存' do
    before do
      ...
      item = FactoryBot.create(:item, user: user_id)
    ...
    context '内容に問題がある場合' do
      ...
      it 'phoneが12文字以上だと保存できないこと' do
        ...
        @order.valid?
        expect(@order.errors.full_messages).to include('is too long (maximum is 12 characters)')
      end
      it 'phoneが10文字以下だと保存できないこと' do
        @order.phone = '123456789'
        @order.valid?
        expect(@order.errors.full_messages).to include('is too short (minimum is 10 characters)')
      end
